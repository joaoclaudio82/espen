/**
 * Importação de planilhas (XLSX) — matriz, ações, trilhas.
 *
 * Funções deste módulo fazem uma detecção heurística do cabeçalho (planilhas
 * com título nas primeiras linhas), normalizam nomes de coluna, mapeiam para
 * a estrutura interna e validam linha por linha.
 *
 * `XLSX` é a lib SheetJS carregada como dep npm e exposta em `window.XLSX`
 * (em main.js). Acesso via window preserva o caminho original.
 */
import { STORAGE_KEYS, getStorage, setStorage } from '../api/storage.js';
import { isAdminUser } from '../auth/roles.js';
import { syncEixoTematicoLegado } from '../shared/acoes-utils.js';
import { genId, idEquals, normalizeActionCode } from '../shared/format.js';
...
Object.assign(globalThis, { importExcelData });
